...
long isLessOrEqual(long x, long y) {
    ...
    long mark = ~((x ^ y) >> 63);
    long eq = !(x ^ y);
    return sign | (((mark & ((x + ~y + 1)) >> 63) | eq) & 1);
}
/*
...
long bitMask(long highbit, long lowbit) {
    return ((~0L) << lowbit) & ((~0L) + (1L << highbit << 1));
}
// 4
/*
...
long trueFiveEighths(long x) {
    long eight = x >> 3;
    long remain = x & 7;
    long eight_mul_5 = (eight << 2) + eight;
    long remain_mul_5 = (remain << 2) + remain;
    return eight_mul_5 + ((remain_mul_5 + (x >> 63 & 7L)) >> 3);
}
/*
